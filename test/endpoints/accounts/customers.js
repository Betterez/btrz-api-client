...
describe("accounts/customers/:id/travellers", () => {
  const token = "I owe you a token";
  const jwtToken = "I owe you a JWT token";
  ...
  it("should GET a list of the customer travellers", () => {
    const customerId = "customer123";
    axiosMock.onGet(`/customers/${customerId}/travellers`).reply(expectRequest({statusCode: 200, token}));
    return api.accounts.customers.travellersAll({customerId, token})
      .then((httpResponse) => {
        expect(httpResponse.status).eql(200);
      });
  });

  it("should GET the traveller with travellerId from the customer", () => {
    const customerId = "customer123";
    const travellerId = "traveller456";
    axiosMock.onGet(`/customers/${customerId}/travellers/${travellerId}`).reply(expectRequest({statusCode: 200, token}));
    return api.accounts.customers.travellersGet({customerId, travellerId, token})
      .then((httpResponse) => {
        expect(httpResponse.status).eql(200);
      });
  });

  it("should POST a new traveller for the customer", () => {
    const customerId = "customer123";
    const data = {
      firstName: "niceName"
    };
    axiosMock.onPost(`/customers/${customerId}/travellers`).reply(expectRequest({statusCode: 201, token, jwtToken}));
    return api.accounts.customers.travellersAdd({customerId, token, jwtToken, data})
      .then((httpResponse) => {
        expect(httpResponse.status).eql(201);
      });
  });

  it("should PUT and update the traveller for the customer", () => {
    const customerId = "customer123";
    const travellerId = "customer123";
    const data = {
      firstName: "anotherNiceName"
    };
    axiosMock.onPut(`/customers/${customerId}/travellers/${travellerId}`).reply(expectRequest({statusCode: 200, token, jwtToken}));
    return api.accounts.customers.travellersUpdate({customerId, travellerId, token, jwtToken, data})
      .then((httpResponse) => {
        expect(httpResponse.status).eql(200);
      });
  });

  it("should DELETE the traveller for the customer", () => {
    const customerId = "customer123";
    const travellerId = "customer123";

    axiosMock.onDelete(`/customers/${customerId}/travellers/${travellerId}`).reply(expectRequest({statusCode: 204, token, jwtToken}));
    return api.accounts.customers.travellersRemove({customerId, travellerId, token, jwtToken})
      .then((httpResponse) => {
